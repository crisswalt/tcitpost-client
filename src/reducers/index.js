...
function postsReducer(state = initialPosts, action)
{
  switch (action.type) {
    ...
    case actionsType.LIST_ALL_POSTS_SUCCESS: // success list all
      return Object.assign({}, state, { isLoading: false, items: action.posts });
    case actionsType.ADD_POST_SUCCESS:       // success add post
      return Object.assign({}, state, { isLoading: false, items: [ ...state.items, action.post ] });
    case actionsType.REMOVE_POST_SUCCESS:    // success remove post
      return Object.assign({}, state, { isLoading: false, items: state.items.filter( item => item.id !== action.post.id ) });
